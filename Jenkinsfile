pipeline {
    ...
    stages {
        ...
        stage('docker-run') {
            ...
            steps {
                sh 'docker stop tomcat-prod || true && docker rm tomcat-prod || true'
                sh 'docker run -d --name tomcat-prod -p 8095:8090 futuretechdevops/tomcat:\$BUILD_NUMBER-\$RELEASE_VERSION'
            }
        }
    }
}
